...
def parser(input_file, database_file):
        ...
        if line.find("seeds:") > -1:
            ...
            for item_number, seed in enumerate(seed_list):
                if item_number % 2 == 0:
                    start_range = seed
                    continue
                for seed_list in range(int(start_range), int(start_range) + int(seed)):
                    seed_object = Almanac(int(seed_list))
                    almanac_list.append(seed_object)
            #         cursor.execute('''INSERT OR IGNORE INTO almanac VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
            #                        seed_object.get_all_properties())
            # connection.commit()
        # except sqlite3.Error as error:
        #     print("Error while connecting to sqlite", error)
        # finally:
    ...
    # from almanac_list find the entry with the smallest location value
    smallest_location = 0
    for almanac_object in almanac_list:
        if almanac_object.get_property("LOCATION") < smallest_location or smallest_location == 0:
            smallest_location = almanac_object.get_property("LOCATION")
